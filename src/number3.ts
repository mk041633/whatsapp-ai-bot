...
const messageQueue3 = new Queue<MessageJobType>('message-queue3', { redis: Config.redisUrl });
...
async function start() {
  ...
  whatsAppClient.on("message", async (msg:Message) => {
            ...
            await messageQueue3.add({ chatId, text });
  });
}

messageQueue3.process(async (job: { id: any; data: { chatId: any; text: any }; }) => {
