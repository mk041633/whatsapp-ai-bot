...
const messageQueue2 = new Queue<MessageJobType>('message-queue2', { redis: Config.redisUrl });
...
async function start() {
  ...
  whatsAppClient.on("message", async (msg:Message) => {
            ...
            await messageQueue2.add({ chatId, text });
  });
}

messageQueue2.process(async (job: { id: any; data: { chatId: any; text: any }; }) => {
